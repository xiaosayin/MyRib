...
class LIDCTwoClassDataset(Dataset):
    def __init__(self, crop_size, move, data_path, train=True, copy_channels=True):
        ...
        if train:
            ...
            self.names = pd.read_csv(os.path.join(data_path, 'info/train_info.csv'))['index'][:-4].dropna()
        else:
    ...
    def __getitem__(self, index):
        with np.load(os.path.join(self.data_path, 'nodule', self.names[index])) as npy:
            return self.transform(npy), self.info.loc[self.names[index][:-4], 'label_code']
